...
| `title` | string | 작업 제목 | `사용자 인증 구현` |
| `description` | string | 상세 설명 | `JWT 기반 로그인/로그아웃` |
| `status` | enum | 작업 상태 | `pending` |
| `priority` | enum | 우선순위 | `high` |
| `specPath` | string\|null | 스펙 문서 경로 | `docs/auth-spec.md` |
| `relatedFiles` | string[] | 관련 파일 목록 | `["src/auth/"]` |
| `notes` | string | 추가 메모 | `보안 검토 필요` |
