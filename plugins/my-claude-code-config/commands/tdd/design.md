...
- **선택 스킬**: `domain-invariant-pattern` - 구현 시 Refactor 단계에서 참조
...
⚠️ 함수명/시그니처/의존성 구조는 설계하지 않는다. 구현 시 TDD Refactor 단계에서 `domain-invariant-pattern` 스킬을 참조하여 결정.
...
|---------|------------|----------|----------|-----|
| BR-1 | {Entity} | 상태 조건 | {자연어 설명} | #1,#2 |
| BR-2 | {Entity} | 행동 제약 | {자연어 설명} | #3 |
| BR-3 | {Entity1, Entity2} | 결과 규칙 | {자연어 설명} | #4,#5 |
```
